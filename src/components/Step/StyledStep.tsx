...
`
export const StepIcon = styled.div<Pick<StepProps, 'small' | 'vertical'> & {isIcon: boolean, state: number, vertical: boolean}>`
  ${({theme, isIcon, small, state, vertical}) => css`
    display: flex;
    justify-content: center;
    align-items: center;

    padding: ${theme.Step["padding-1.5"] + 'rem'} ${isIcon ? theme.Step["padding-1.5"] + 'rem' : theme.Step["padding-2.5"] + 'rem'};
    ${small && css`padding: ${theme.Step["padding-1"] + 'rem'} ${isIcon ? theme.Step["padding-1"] + 'rem' : theme.Step["padding-2"] + 'rem'};`}
...
export const StepContent = styled.div<{isLast: boolean, vertical: boolean}>`
  ${({theme, isLast, vertical}) => css`
    ${isLast && css`min-width: ${theme.Step.descriptionWidth + 'px'};`};
    ${vertical ?
            css`
              overflow: hidden;
            `
...
export const StepTitle = styled.div<Pick<StepProps, 'small'> & {state: number}>`
  ${({theme, small, state}) => css`
    ...
    color: ${state == State.LESS || state == State.CURRENT ? theme.Step.colorTextBase : theme.Step.colorTextTertiary};
    white-space: nowrap;
    height: 100%;
  `}
`
