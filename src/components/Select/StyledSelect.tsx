...
export const OptionsList = styled.ul<SelectStyledProps & {isOpen: boolean}>`
  ${({theme, isOpen, shape, type}) => css`
    ...
    ${type !== 'primary' && css`border: ${theme.Select.borderWidth} ${theme.Select.borderStyle} ${getColor(type as Status, 'Select')};`}
    ${shape && setShape(shape,'Select')};
...
export const OptionItem = styled.li<OptionItemProps>`
  ${({theme, highlighted, isSelected, isDisabled, type}) => css`
    ...
    ${highlighted && css`background-color: ${theme.Select.colorHoverBase};`}
    ${isSelected && css`background-color: ${getColor(type as Status, 'Select', {isShadow: true})}`}
    ${isDisabled && css`
