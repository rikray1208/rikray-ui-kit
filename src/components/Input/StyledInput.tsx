...
function getColor(type: InputType, isShadow?: boolean) {
    if (isShadow) {
        switch (type) {
            case 'success':
                return css`
                    ${({ theme }) => theme.Input.colorShadowSuccess};
                `;
            case 'warning':
                return css`
                    ${({ theme }) => theme.Input.colorShadowWarning};
                `;
            case 'error':
                return css`
                    ${({ theme }) => theme.Input.colorShadowError};
                `;
            case 'base':
                return css`
                    ${({ theme }) => theme.Input.colorShadowPrimary};
                `;
        }
    }
    ...
    switch (type) {
        case 'success':
            return css`
                ${({ theme }) => theme.Input.colorSuccess};
            `;
        case 'warning':
            return css`
                ${({ theme }) => theme.Input.colorWarning};
            `;
        case 'error':
            return css`
                ${({ theme }) => theme.Input.colorError};
            `;
        case 'base':
            return css`
                ${({ theme }) => theme.Input.colorPrimary};
            `;
    }
}
...
function setPadding(size: Size) {
    if (size == 'Small')
        ...
        `;
    return css`
        padding: ${({ theme }) => theme.Input['padding-2'] + 'rem' + ' ' + theme.Input['padding-4'] + 'rem'};
    `;
}
...
export const StyledPrefix = styled.div<Pfix>`
    ${({ theme }) => css`
        ...
        border-radius: ${theme.Input['borderRadius-lg'] +
        'px' +
        ' ' +
        '0' +
        ' ' +
