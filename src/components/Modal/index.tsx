...
const Modal: FC<PropsWithChildren<ModalProps>> = (props) => {
    ...
    return (
        <>
            {open &&
                createPortal(
                    <div role={'button'} tabIndex={0} ref={modalRef}>
                        {overlay && (
                            <Overlay id='rikray-overlay' className={!open ? 'fade-out' : ''} onClick={onCancel} />
                        )}
                        <StyledModal
                            id='rikray-modal'
                        ...
                        >
                            ...
                            {children}
                            <Footer>
                                <Button shape={shape} onClick={onCancel} variant={'Outline'} {...btnCansel}>
                                    {cancelText}
                                </Button>
                                <Button shape={shape} onClick={onConfirm} {...btnConfirm}>
                                    {confirmText}
                                </Button>
                            </Footer>
                        </StyledModal>
